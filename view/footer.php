<!DOCTYPE html>

<!--*************** PIED DE PAGE ***************-->
<footer id="footer" style="background-color:#4e4e4e;color:white">
    <ul class="footer-links">
        <li class="footer-item"><b>NOUS CONTACTER :</b></li>
        <li class="footer-item">EMAIL : scierie.gineste@wanadoo.fr</li>
        <li class="footer-item">TELEPHONE : +33 9 70 35 54 09</li>
        <li class="footer-item">ADRESSE : Route de Rodez, 12220, MONTBAZENS </li>
        <li class="footer-item">NOUS SUIVRE
            <a href="https://www.facebook.com/Scierie-du-Fargal-613509152159633/" target="_blank">
                <img src="images/facebook.webp" alt="Logo de Facebook" />
            </a>
        </li>
    </ul>
</footer>
